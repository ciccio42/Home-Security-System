...
static void convert_buffer_input(date_time_t *date_time, uint8_t *buffer){

	date_time->seconds = 10*(buffer[0]>>4 & 0x7) + (buffer[0] & 0xF);
	date_time->minutes = 10*(buffer[1]>>4 & 0xF) + (buffer[1] & 0xF);
	date_time->hours = 10*(buffer[2]>>4 & 0xF) + (buffer[2] & 0xF);
	date_time->day = 10*(buffer[3]>>4 & 0x3) + (buffer[3] & 0xF);
	date_time->date = 10*(buffer[4]>>4 & 0xF) + (buffer[4] & 0xF);
	date_time->month = 10*(buffer[5]>>4 & 0xF) + (buffer[5] & 0xF);
	date_time->year = 10*(buffer[6]>>4 & 0xF) + (buffer[6] & 0xF);
...
int8_t ds1307rtc_update_date_time(rtc_t *rtc){
	...
	convert_buffer_input(&(rtc->date_time), input_buffer);
	return DS1307_OK;
